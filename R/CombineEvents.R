...
CombineEvents <- function(dat) {
  ...
  for (trial in 1:length(dat$item)) {
    ...
    dat$item[[trial]]$fix2 <- data.frame(matrix(NA, nrow(dat$item[[trial]]$fix) , 9))
    colnames(dat$item[[trial]]$fix2) <- c("num", "start", "stop", "xs", "ys", "xe", "ye", "ps", "msg")
    ...
    dat$item[[trial]]$fix2$ps <- dat$item[[trial]]$fix$ps
    ...
    dat$item[[trial]]$sac2$ps <- NA
    names = c("sacid", "start", "stop", "xs", "ys", "xe", "ye", "ps", "msg")
    dat$item[[trial]]$sac2 <- dat$item[[trial]]$sac2[, match(names, colnames(dat$item[[trial]]$sac2))]
    colnames(dat$item[[trial]]$sac2) <- c("num", "start", "stop", "xs", "ys", "xe", "ye", "ps", "msg")
    
    # msg
    # ----
    
    dat$item[[trial]]$msg2 <- data.frame(matrix(NA, nrow(dat$item[[trial]]$msg), 9))
    colnames(dat$item[[trial]]$msg2) <- c("num", "start", "stop", "xs", "ys", "xe", "ye", "ps", "msg")
    
    dat$item[[trial]]$msg2$num <- 1:nrow(dat$item[[trial]]$msg)
    dat$item[[trial]]$msg2$start <- dat$item[[trial]]$msg$time
    dat$item[[trial]]$msg2$ps <- NA
    ...
    names <- c("num", "msgnum", "fixnum", "sacnum", "msg", "start", "stop", "dur", "ps", "xs", "ys", "xe", "ye")
