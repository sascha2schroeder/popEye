...
SetupExperiment <- function(env = parent.frame(n = 1)) {
  ...
  if (font$name == "Arial" & font$size == 20) {
    ...
    pixel <- c(15,15,14,15,15,7,15,15,6,6,14,6,22,
               ...
               11,15,12,9,9,8,27,16,17, 5, 8, 8, 9)
    font$letpix <- data.frame(letter = letter, pixel = pixel)
    font$height <- 25
    font$width <- 14.5
    font$lead <- 6
  ...
  if (font$name == "Arial" & font$size == 22) {
    ...
    pixel <- c(16,16,15,16,16,8,16,16,7,7,14,7,25,
               ...
               12,16,14,10,10,8,29,17,18, 6, 8, 8, 10)
    font$letpix <- data.frame(letter = letter, pixel = pixel)
    font$height <- 27
    font$width <- 15.5
    font$lead <- 7
  ...
  # CourierNew, 22 pt
  if (font$name == "CourierNew" & font$size == 22) {
  ...
  if (font$name == "Adyghe" & font$size == 20) {
    letter <- c(" ", "-", "\u2013", ",", ":", ".", "\u00ab", "\u00bb", "(", ")", 
                ...
                "\u044f", "\u042f", "\u04c0", "\u20de",
                "\u0964")
    pixel <- c(18,18,18,18,18,18,18,18,18,18,
               18,18,18,18,18,18,18,18,18,18,
               18,18,18,18,18,18,18,18,18,18,
               18,18,18,18,18,18,18,18,18,18,
               18,18,18,18,18,18,18,18,18,18,
  ...
  # Consolas, 26 pt
  if (font$name == "Consolas" & font$size == 26) {
  ...
  if (font$name == "TimesNewRoman" & font$size == 22) {
    letter <- c("a", "\u00e5", "\u00e6", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m",
                ...
                "&","/","(",")","$","[","]","=","\",", "\u00A7","'","<",">","|",
                "°","\u20AC","^","{","}","\u0020","@","\u00B5","ß")
    pixel <- c(12,12,19,14,12,14,13,9,13,14,8,8,14,8,22,
               14,14,15,14,14,9,11,8,14,14,20,14,13,13,
               12,14,14,
               20,18,19,21,18,17,20,20,10,11,20,17,26,
               21,20,16,20,19,16,18,21,21,27,21,21,17,
               20,20,26,20,20,21,
               14,14,14,14,14,14,14,14,14,14,
               7,7,8,8,9,9,15,15,15,16,16,10,8,12,24,
  ...
  # Tahoma, 20 pt
  if (font$name == "Tahoma" & font$size == 20) {
    ...
    pixel <- c(14, 15, 12, 15, 14, 9, 15, 15, 6, 8, 13, 6, 23, 
               15, 15, 15, 15, 10, 12, 9, 15, 13, 20, 13, 13, 12, 
               14, 15, 15, 
               16, 16, 16, 18, 15, 14, 18, 18, 10, 11, 16, 13, 21, 
               18, 19, 15, 19, 17, 15, 16, 18, 16, 24, 16, 16, 15, 
               16, 19, 18, 
               15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 
               8, 8, 10, 10, 10, 10, 15, 20, 15, 20, 20, 11, 9, 13, 26, 
               18, 10, 10, 10, 15, 10, 10, 20, 11, 11, 11, 15, 6, 6, 20, 20, 10, 
               13, 15, 20, 13, 13, 8, 25, 15, 15, 20, 
               6, 12, 12, 12, 15, 12, 11, 15, 15)
  ...
  # Song 22 pt
  if (font$name == "Song" & font$size == 22) {
    font$letpix <- data.frame(letter = " ", pixel = 29)
    font$height <- 29
    font$width <- 29
    font$lead <- 4
    font$right <- FALSE
  ...
  if (font$name == "HindiKokila" & font$size == 28) {
    ...
    lig_pixel <- c(14, 17, 7, 11, 14, 19, 14, 14, 13, 14, 14, 
                   15, 16, 16, 15, 15, 11, 10, 12, 14, 12, 8, 
                   8, 13, 12, 12, 10, 10, 0, 14, 10, 11, 9, 12, 13)
