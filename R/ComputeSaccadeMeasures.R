...
ComputeSaccadeMeasures <- function(dat, trial, env = parent.frame(n = 2)) {
  ...
  for (s in 1:nrow(sac)) {
   # s = 1
    ...
    sac$dist.line[s] <- sac$linee[s] - sac$lines[s]
    ...
    if (length(dat$item[[trial]]$xy$x[dat$item[[trial]]$xy$time >= a & dat$item[[trial]]$xy$time <= b]) > 0 & 
      ...
      if (is.na(dat$item[[trial]]$meta$calibration.method) == F) {
        ...
        if (dat$item[[trial]]$meta$calibration.method != "H3") {
          sac$pv.px[s] <- round(max(sqrt(dat$item[[trial]]$vxy$x[dat$item[[trial]]$vxy$time >= a &
        ...
        sac$dX[s] <- round(sign(ix2 - ix1) * (maxx - minx), 1)
        if (dat$item[[trial]]$meta$calibration.method != "H3") {
          sac$dY[s] <- round(sign(iy2 - iy1) * (maxy - miny), 1)
        }
  ...
  #            "amp", "pv", 
  #            "lines", "linee", "dist.line", "lets", "lete", "dist.let", 
