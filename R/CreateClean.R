...
CreateClean <- function(dat, env = parent.frame(n = 1)) {
  ...
  if (env$exp$setup$type == "target"  | env$exp$setup$type == "boundary" | env$exp$setup$type == "fast") {
    ...
    cleantmp$target.pre.sac <- unlist(lapply(lapply(lapply(dat$item, "[[", "clean"), "[[", "target"), "[[", "pre.sac"))
    ...
    cleantmp$target.post.sac <- unlist(lapply(lapply(lapply(dat$item, "[[", "clean"), "[[", "target"), "[[", "post.sac"))
    cleantmp$target.post.refix <- unlist(lapply(lapply(lapply(dat$item, "[[", "clean"), "[[", "target"), "[[", "post.refix"))
