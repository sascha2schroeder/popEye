...
ComputeSentenceMeasures <- function(fix, sent.item) {
  ...
  # NOTE: Be careful: Regressions etc. on fixation-level due not correspond to sentence-level measures anymore
   ...
   for (i in 1:max(fixin$trialnum)) {
     # i <- 1
     
     for (j in 2:(nrow(fixin[fixin$trialnum == i, ]) - 2)) {
       # j <- 13
       
       if (fixin$sentnum2[fixin$trialnum == i][j] != fixin$sentnum2[fixin$trialnum == i][j - 1]) {
         
         if (fixin$sentnum2[fixin$trialnum == i][j + 1] == fixin$sentnum2[fixin$trialnum == i][j - 1] |  
             fixin$sentnum2[fixin$trialnum == i][j + 2] == fixin$sentnum2[fixin$trialnum == i][j - 1]) {
           fixin$sentnum2[fixin$trialnum == i][j] <- fixin$sentnum2[fixin$trialnum == i][j - 1]
         }
  ...
  # compute indicator
  fixin$id <- as.character(paste(fixin$trialnum, fixin$sentnum2, sep = ":"))
  ...
  for (i in 1:max(fixin$trialnum)) {
    ...
    fixin$last[fixin$trialnum == i][1] <- fixin$id[fixin$trialnum == i][1]
    
    for (j in 2:nrow(fixin[fixin$trialnum == i, ])) {
      # j <- 1
    
      # compute last
      fixin$last[fixin$trialnum == i][j] <- fixin$id[fixin$trialnum == i][j - 1]
      
      
      # compute regressions
      if(fixin$sentnum2[fixin$trialnum == i][j] < fixin$sentnum2[fixin$trialnum == i][j - 1]) {
        
        fixin$sent.reg.in2[fixin$trialnum == i][j] <- 1
        fixin$sent.reg.out2[fixin$trialnum == i][j - 1] <- 1
        fixin$sent.reg.in.from2[fixin$trialnum == i][j] <- fixin$sentnum2[fixin$trialnum == i][j - 1]
        fixin$sent.reg.out.to2[fixin$trialnum == i][j - 1] <- fixin$sentnum2[fixin$trialnum == i][j]
        
      }
      
      # compute runid
      if (fixin$sent.reg.in2[fixin$trialnum == i][j] == 1 & fixin$sent.reg.in2[fixin$trialnum == i][j - 1] != 1) {
        fixin$sent.runid2[fixin$trialnum == i][j] <- fixin$sent.runid2[fixin$trialnum == i][j - 1] + 1
      } else {
        fixin$sent.runid2[fixin$trialnum == i][j] <- fixin$sent.runid2[fixin$trialnum == i][j - 1]
      }
      
      # compute forward
      if (fixin$wordnum[fixin$trialnum == i][j] >= fixin$wordnum[fixin$trialnum == i][j-1] ) {
        fixin$forward[fixin$trialnum == i][j] <- 1
      }
  ...
  for (i in 1:max(sent$trialnum)) {
    # i <- 10
    
    mem <- NULL
    
    for (j in 1:nrow(sent[sent$trialnum == i, ])) {
      # j = 1
      
      if (is.null(mem) == F) {
        
        if (is.null(mem) == F & sent$sentnum[sent$trialnum == i][j] < max(mem)) {
          sent$firstrun.skip[sent$trialnum == i][j] <- 1
        }
        
      }
      
      if (sent$sentnum[sent$trialnum == i][j] %in% mem == F) {
        mem <- c(mem, sent$sentnum[sent$trialnum == i][j])
      }
  ...
  # total
  tmp <- aggregate(fixin$fixid, list(fixin$id), length)
  ...
  # firstpass
  tmp <- aggregate(fixin$fixid[fixin$firstpass == 1], list(fixin$id[fixin$firstpass == 1]), length)
  ...
  # firstpass-forward
  tmp <- aggregate(fixin$fixid[fixin$firstpass == 1 & fixin$forward == 1], list(fixin$id[fixin$firstpass == 1 & fixin$forward == 1]), length)
  ...
  # firstpass-reread
  tmp <- aggregate(fixin$fixid[fixin$firstpass == 1 & fixin$forward == 0], list(fixin$id[fixin$firstpass == 1 & fixin$forward == 0]), length)
  ...
  # firstrun$firstrun.refix <- as.numeric(tapply(firstruntmp$refix, list(firstruntmp$id), max))
  ...
  # senttmp$blink <- as.numeric(tapply(senttmp$blink, list(senttmp$id), max))
  # senttmp$refix <- as.numeric(tapply(senttmp$sent.refix, list(senttmp$id), max, na.rm = T))
  ...
  # # recompute blinks
  # sent$blink[is.na(sent$total.dur)] <- 0
