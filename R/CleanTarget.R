...
CleanTarget <- function(dat, env = parent.frame(n = 2)) {
  ...
  for (trial in 1:length(dat$item)) {
    ...
    if (is.na(target.min) == F) {
      min.range <- seq(from = min(dat$item[[trial]]$meta$stimmat$xs[dat$item[[trial]]$meta$stimmat$ianum == target.min]),
                       to = max(dat$item[[trial]]$meta$stimmat$xe[dat$item[[trial]]$meta$stimmat$ianum == target.min]))
    } else {
    ...
    if (target.end$sac.out < 0) {
      dat$item[[trial]]$clean$target$post.sac <- 1
    }
    # NOTE: useful?
