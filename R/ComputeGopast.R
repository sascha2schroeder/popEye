...
ComputeGopast <- function(dat) {
  ...
  # for (i in 1:1){
    ...
    ias <- as.numeric(unlist(dimnames(table(dat$ia[id == ids[i]]))))
    ...
    # for (j in 2:2){
      # j = 1
        ...
        dat$gopast[id == ids[i]][dat$ia[id == ids[i]]== ias[j]] <- 
          sum(
            dat$dur[id == ids[i]][
              dat$fixid[id == ids[i]] >= min(dat$fixid[id == ids[i]][dat$ia[id == ids[i]] == ias[j]]) 
              & dat$fixid[id == ids[i]] < min(dat$fixid[id == ids[i]][dat$ia[id == ids[i]] > ias[j]])
              ]
            )
        
        dat$selgopast[id == ids[i]][dat$ia[id == ids[i]]== ias[j]] <- 
          sum(
              dat$dur[id == ids[i]][
                dat$fixid[id == ids[i]] >= min(dat$fixid[id == ids[i]][dat$ia[id == ids[i]] == ias[j]])
                & dat$fixid[id == ids[i]] < min(dat$fixid[id == ids[i]][dat$ia[id == ids[i]] > ias[j]])
                & dat$ia[id == ids[i]] == ias[j]
                ]
            )
        # print(j)
    }
